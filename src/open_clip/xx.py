...
class Revised_Transformer(nn.Module):
    def __init__(self,
            width: int,
            layers: int,
            heads: int,
            ...
            ls_init_value: float = None,
            act_layer: Callable = nn.GELU,
            norm_layer: Callable = LayerNorm, prompts_needed=0,
        ...
        print("222 design_details", design_details)
        ...
        if current_trainer == 'MaPLe':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, mlp_ratio, design_details, text_layer, i, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer, )
                  for i in range(layers)])
        else:
            self.resblocks = nn.ModuleList([
            ResidualAttentionBlock(
    ...
    def forward(
        ...
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0:
                ...
                print("counter", counter, len(compound_prompts_deeper))
...
class Revised_VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        print("111 design_details", design_details)
        self.transformer = Revised_Transformer(
            width,
            layers,
            heads,
            mlp_ratio,
    ...
    def forward(self, x: torch.Tensor, compound_deeper_prompts, mask = None, proj = False):
        ...
        for mask_scale in mask:
            ...
            # print("mask_scale", mask_scale.shape)
            ...
            # x_scale.append(torch.index_select(x, 1, mask_scale))
            # print("mask", x.shape)
            ########################################


            # print("cat", x_scale.shape)
            ...
            outputs = self.transformer([x_scale, compound_deeper_prompts, 0])
            x_scale = outputs[0]
    ...
    #######################################################################     
        ...
        if self.output_tokens:
            return pooled, tokens
        # print("tokens" ,pooled.shape, x[:, 1:].float().shape, (tokens@ self.proj).shape)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask = None, proj = False):
        # to patches - whether to use dual patchnorm - https://arxiv.org/abs/2302.01327v1
        b, c, _, _  = x.shape
        if self.input_patchnorm:
            # einops - rearrange(x, 'b c (h p1) (w p2) -> b (h w) (c p1 p2)')
...
class Revised_TextTransformer(nn.Module):
    output_tokens: torch.jit.Final[bool]
    ...
    def forward(self, text):
        ...
        if self.output_tokens:
            return pooled, tokens
        print("textpeeeeeeeeexxxxxxxxxxxxxxxxxxxeeeeeeed",pooled.shape, tokens.shape)
        return pooled
##########################################################################################################################
...
###############################################
class CUTOMTextTransformer(nn.Module):
    output_tokens: torch.jit.Final[bool]
